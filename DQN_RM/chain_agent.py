...
class Agent:
    ...
    def select_action(self, state):
        ...
        eps_threshold = self.eps_end + (self.eps_start - self.eps_end) * math.exp(-1.*self.env.nsteps/self.eps_decay)
    ...
    def train(self):
        ...
        torch.save(self.policy_net.state_dict(), './trained_models/chain_agent.pt')
        # print('AUC: {}'.format(sum(self.cum_rewards) / len(self.cum_rewards)))

    def load_weights(self):
        self.policy_net.load_state_dict(torch.load('./trained_models/chain_agent.pt'))
