...
def evaluate_performance(args, seeds=(1, 2, 3, 4, 5)):
    ...
    for seed in seeds:
        ...
        # Save CSV file
        f = open('./results/seed{}_Double_DQN_Chain_performance.csv'.format(seed), 'w', newline='')
        writer = csv.writer(f)
        for item in pf:
...
def evaluate_sample_efficiency(args, seeds=(1, 2, 3, 4, 5)):
    ...
    for seed in seeds:
        ...
        # Save CSV file
        f = open('./results/seed{}_Double_DQN_Chain_sample_efficiency.csv'.format(seed), 'w', newline='')
        writer = csv.writer(f)
        for item in se:
            writer.writerow([item])
        f.close()
